...
def main(args):
    if not args.train_url:
        mox.file.copy(args.eval_from,'/tmp/model/'+args.eval_from.split('/')[-1])
#         args.eval_from = '/tmp/model/'+args.eval_from.split('/')[-1]+'.tar'
#         print('**********************',args)
        args.eval_from = '/tmp/model/'+args.eval_from.split('/')[-1]
#     save_dict = torch.load(args.eval_from, map_location='gpu')
#     print(save_dict)
    ...
    print('#####################')
    print(args)
#     with open(args.eval_from,'rb') as f:
#         buffer = io.BytesIO(f.read())
#         save_dict = torch.load(buffer)
    save_dict = torch.load(args.eval_from, map_location='cpu')
    state_dict = save_dict['state_dict']

    for k in list(state_dict.keys()):
        if k.startswith('module.backbone'): #and not k.startswith('module.encoder_q.fc'):
